...
class Graph:
    ...
    def add_edge(self, source: str, target: str) -> None:
        ...
        if (source,target) and (target,source) in self.edges:
            None
        elif (source,target) and (target,source) not in self.edges:
    ...
    def max_degree_nodes(self) -> dict:
        ...
        """
        max_ID = self.nodes[0]
        Tracker={} #tracker variable set to keep track of all the nodes with highest degree
        degree_max=1 #initializing the max degree as 1 as it is tge bas case for a graph
        for edge in self.edges:
            starting_node = edge[0]
            if starting_node in Tracker.keys():
                Tracker[starting_node] += 1
                if Tracker[starting_node] > degree_max:
                    degree_max = Tracker[starting_node]
                    
            else:
                Tracker[starting_node] = 1
            target = edge[1]
            if target in Tracker.keys():
                Tracker[target] = Tracker[target]+ 1
                if Tracker[target] > degree_max:
                    degree_max = Tracker[target] 
                
            else:
                Tracker[target] = 1
        result = {} #resultant dictionary which contains the nodes with the highest degree
        for id in Tracker.keys():
            if Tracker[id] == degree_max:
                result[id] = degree_max 
    ...
    def build_graph(self,list_of_movies,exclist,utils, ID_per_person):
        for movie in list_of_movies:
            cast_members = utils.get_movie_cast(movie_id=str(movie['id']),limit=3, exclude_ids=exclist)
            for cast in cast_members:
                castID = cast['id']
                castName = cast['name']
                graph.add_node(str(castID), castName)
                graph.add_edge(str(ID_per_person), str(castID))
...
class  TMDBAPIUtils:
    ...
    def get_movie_cast(self, movie_id:str, limit:int=None, exclude_ids:list=None) -> list:
        ...
        if r1.status == 200:
            info = r1.read()
            data = json.loads(info.decode('ascii', 'ignore'))
        
        casts = []
        for cast in data['cast']:
            if exclude_ids is not None and cast['id'] in exclude_ids:
                continue
            cast['name'] = cast['name'].replace(',','')

            if limit is not None and cast['order'] < limit:
                casts.append(cast)
            if limit is None:
                casts.append(cast)
    ...
    #     """ 
        ...
        for credit in data['cast']:
            ...
            elif credit['vote_average'] >=vote_avg_threshold:
                credits.append(credit)
...
def return_argo_lite_snapshot()->str:
    ...
    # """
    url="https://poloclub.github.io/argo-graph-lite/#1eca389b-d168-4245-ab5f-52ae177efa80"
...
if __name__ == "__main__":
    ...
    for i in range(0,3):
        ...
        for node in nodes:
            n+=1
            ID_per_person = node[0]
            Name_per_person = node[1]
            movies = tmdb_api_utils.get_movie_credits_for_person(ID_per_person,vote_avg_threshold=8.0) #   Find all of Laurence Fishburne's movie credits that have a vote average >= 8.0
            List_to_avoid.append(int(ID_per_person))
            graph.build_graph(movies,List_to_avoid,tmdb_api_utils,ID_per_person) #method that retrieves data for each movie
